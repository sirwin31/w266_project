...
TRAIN_DATA = "train_encodings_23Nov_gcpf_distilbert.pickle"
...
CHECKPOINT_FILE = "saved_model_epoch5_20201123_1524.tar"
...
def train():
    ...
    # Load training data
    train_dataset = util.data.load_dataset_from_path(TRAIN_DATA)
    ...
    for epoch in range(1 + prior_epochs, EPOCHS + 1 + prior_epochs):
        ...
        for batch_num, batch in enumerate(train_loader):
            ...
            if (batch_num + 1) % BATCH_REPORT_FREQ == 0:
                logger.info(f"Epoch: {epoch}, Batch: {batch_num + 1}, loss: {loss}")
            else:
